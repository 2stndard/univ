...
df |>
  ...
  geom_line(data = df.미래 |> filter(연도 <= 2032), aes(x = as.factor(연도+1), y = 학생수, group = 1, color = '전년 고3생수')) + 
  geom_line(data = df.미래 |> filter(연도 > 2032), aes(x = as.factor(연도+1), y = 학생수, group = 1, color = '전년 고3생수'), linetype = 2) + 
...
#  scale_color_manual(name = '구분', values = c('입학자수' = 'grey50', '전체정원' = 'red', '전년 고3생수' = 'black', '21년정원' = 'red4')) +
  scale_shape_manual(name = '구분', values = c('입학자수' = 1, '전체정원' = 2, '전년 고3생수' = 3, '21년정원' = 4, '고3생수 추세선' = 0)) +
  scale_size_manual(name = '구분', values = c('입학자수' = 2, '전체정원' = 2, '전년 고3생수' = 2, '21년정원' = 2, '고3생수 추세선' = 0)) + 
  scale_y_continuous(labels = scales::comma) +
  theme_bw() +
  theme(legend.position = 'bottom') + 
  geom_smooth(data = df.미래, aes(x = 연도-2011+1, y = 학생수), formula = y ~ x, method = 'loess') + 
  scale_color_manual(name = '구분', values = c('입학자수' = 'grey50', '전체정원' = 'red', '전년 고3생수' = 'black', '21년정원' = 'red4', '고3생수 추세선' =  'blue')) + 
  ...
  labs(y = '학생수', caption = '※전년도 고3 학생수의 22년도 이후 데이터는 현재 학생수(실선) 및 주민등록인구(점선)기반 추정치임') +
  geom_vline(aes(xintercept = 11), color = 'grey80', linetype = 2) +
  annotate('rect', xmin = 8.5, xmax = 13.5, ymin = 380000, ymax = 580000, alpha = 0.2) +
  annotate('rect', xmin = 24.5, xmax = 27.5, ymin = 260000, ymax = 380000, alpha = 0.2) +
  ...
  annotate('text', x = 23, y = 325000, label = '2차 충격') + 
  annotate('segment', x = 24, y = 325000, xend = 24.5, yend = 325000, arrow = arrow(length = unit(0.01, "npc")))
