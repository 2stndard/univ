...
df |>
  ...
  geom_line(data = df.미래 |> filter(연도 <= 2032), aes(x = as.factor(연도+1), y = 학생수, group = 1, color = '전년 고3생수(22년이후는 추정치)')) + 
  geom_line(data = df.미래 |> filter(연도 > 2032), aes(x = as.factor(연도+1), y = 학생수, group = 1, color = '전년 고3생수(22년이후는 추정치)'), linetype = 2) + 
