...
#  scale_color_manual(name = '구분', values = c('입학자수' = 'grey50', '전체정원' = 'red', '전년 고3생수' = 'black', '21년정원' = 'red4')) +
  scale_shape_manual(name = '구분', values = c('입학자수' = 1, '전체정원' = 2, '전년 고3생수' = 3, '21년정원' = 4, '고3생수 추세' = 0)) +
  scale_size_manual(name = '구분', values = c('입학자수' = 2, '전체정원' = 2, '전년 고3생수' = 2, '21년정원' = 2, '고3생수 추세' = 0)) + 
  scale_y_continuous(labels = scales::comma) +
  theme_bw() +
  theme(legend.position = 'bottom') + 
  geom_smooth(data = df.미래, aes(x = 연도-2011+1, y = 학생수), formula = y ~ x, method = 'loess') + 
  scale_color_manual(name = '구분', values = c('입학자수' = 'grey50', '전체정원' = 'red', '전년 고3생수' = 'black', '21년정원' = 'red4', '고3생수 추세' =  'blue')) + 
